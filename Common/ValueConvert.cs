...
namespace Common
{
    public static class ValueConvert
    {
        ...
        public static List<string> GetIdSort(this string ids)
        {
            ...
            if (!string.IsNullOrEmpty(ids))
            {
                var sort = new SortedSet<string>(ids.Split(',')
                    .Where(w => !string.IsNullOrWhiteSpace(w) && w.Length > 36 && w.Contains('&'))
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static List<Permissions> GetPermissions(this string value)
        {
            var permissions = new List<Permissions>();
            if (string.IsNullOrWhiteSpace(value)) return permissions;
            var fids = value.Split(',');
            permissions.AddRange(from f in fids
                select f.Split('[')
                into tf
                let father = tf[0]
                let child = tf[1].TrimEnd(']').Split(';').ToList()
                select new Permissions
                {
                    Fid = father, Child = child
                });

            return permissions;
        }

        public static List<string[]> SeatStringToDictionary(string value)
        {
            ...
            for (int i = 0; i < s.Length; i++)
            {
                ...
                {
                    var ss = s[i].Split(',');
                    if ((!string.IsNullOrEmpty(ss[0])) && (!string.IsNullOrEmpty(ss[1])) && (!string.IsNullOrEmpty(ss[2])))
                    {
                        list.Add(ss);
                    }
                }
        ...
        public static int GetInt(this object Value, int defaultValue)
        {
            ...
            if ((Value is string) == false && (Value is IConvertible))
            {
        ...
        public static int GetSecond(string Value)
        {
            ...
            return GetInt(arrayStr[0]) * 60 + GetInt(arrayStr[1]);
        }
        ...
        public static string GetSecondTime(DateTime defaultValue)
        {
            DateTime baseDate=new DateTime(1970,1,1);
            TimeSpan diff = defaultValue - baseDate;
            double millsecond = diff.TotalMilliseconds;
            return millsecond.ToString();
        }
        ...
        /// 返回功能权限（菜单-功能Id）字典
        /// </summary>
        /// <param name="functionstring">角色功能权限字符串</param>
        /// <returns></returns>
